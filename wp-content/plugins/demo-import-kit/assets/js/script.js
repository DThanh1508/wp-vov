jQuery( function ( $ ) {
	...
	var dropdownButton = document.getElementsByClassName('dik-dropdown-button')[0];
	var dropdownList = document.getElementsByClassName('dik-dropdown-list')[0];
...
// The lines below (inside) are executed on change & keyup
	...
	$( '.action-import-grid' ).on( 'click', function () {
		...
		$('.dik-content-wrapper').addClass('dik-grid-importing');
		$('.dik-content-wrapper').empty();
		$('.dik-content-wrapper').append('<div class="dik-wrapper dik-notification-message"><h2>'+ dik.importing_title+'</h2><p>'+ dik.importing_message+'</p><div class="dik-import-loading"></div></div>');
	...
	function ajaxCall( data ) {
		...
		$.ajax({
			...
			if ( 'undefined' !== typeof response.status && 'newAJAX' === response.status ) {
				...
				if( ! $('.dik-plugin-masthead, .dik-drag-drop').hasClass('dik-demo-importing') ){

					$('.dik-content-wrapper').append('<div class="dik-wrapper dik-notification-message"><h2>'+ dik.import_status+'</h2><p>'+ response+'</p><div class="dik-import-loading"></div></div>');
				}
  ...
  var $filters = $('.dik-header-filter [data-filter]'),
    ...
    $filters.on('click', function(e) {
      ...
      if ($filterClass == 'all') {
        ...
        $boxes.removeClass('is-animated')
          .fadeOut().promise().done(function() {
            $boxes.filter('[data-category = "' + $filterClass + '"]')
